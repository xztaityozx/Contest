	...
	internal class Program {
		...
		private class Calc {
			public void Solve() {
				int A, B, C, D, E, F;
				var read= ReadInts();
				A = read[0]*100; B = read[1]*100;C = read[2];D = read[3];E = read[4];F = read[5];
				//現在の砂糖水のg
				int a = 0, b = 0;
				Li list = new Li();
				...
				REP(F / A+1, x => {
					REP(F / B+1, y => {
						...
						int max = 0;
						bool flag = true;
						for (int k = 0; k <= Smax/C&&flag; k++) {
							for (int v = 0; v <= Smax/D; v++) {
								int sx = k * C + v * D;
								if (sx <= Smax) { max = Max(max, sx); }
								//else { flag = false; break;  }
							}
						}
						if (W+max<=F&&Mol(ans.Item1, ans.Item2) < Mol(W, max)) ans = new Tuple<int, int>(W, max);
					});
				});
				(ans.Item1 + ans.Item2+" "+ans.Item2).WL();
				return;
			}
			private int Mol(int a, int b) => b==0?0: (100 * b) / (a + b);
		}
	}
