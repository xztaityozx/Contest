...
- まず(x,y)をグラフの辺としてみる。グラフがいくつかできる（非連結な場合もあるため）
  - 交換はこのグラフのなかで頂点を入れ替えることと同義になる。
  - ここで重要なこととして1つのグラフの中で任意の頂点はどこにでも行けるということ。
  - つまり目的のindexのついた頂点へ好きに移動できるのでグラフの中に目的の頂点があるかどうかを確かめればいい
- 二つの頂点が連結かどうかはUnion-Findですぐわかるのでやる
  - `uf.IsSameGroup(P[i]-1,i)`が`true`なら答えに1加算する。
  - これをNまで確かめればOK
- まぁUnion-Findやるだけ

# ABC099 D Good Grid
- AtCoder問題文よくわからないコンテスト
  - 時間内にたどり着けなかったょ･･･
- NxNマスのグリッド上で(i+j)%3の値が同じになるマス(i,j)がすべて同じ色になるように塗り替える。
  - 色xから色yへ塗り替えるときにはD[x,y]のコストがかかるとき塗り替えたときの最小コストはいくらか
- まずC色の色のうち3色しか使わない
  - (i+j)%3が同じマスは同じ色であるため
- なので最大でも色の組み合わせは `30x29x28=24360`個。全探索ができそう
