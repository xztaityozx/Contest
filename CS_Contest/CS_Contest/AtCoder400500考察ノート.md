...
- 競プロ辞めたい
- N個の島が橋で一直線につながっている。このとき(a,b)が連結にならないようにするには、橋を壊せばいい。
  - M個の(a,b)の組が与えられるので、条件を満たすのに壊さないといけない橋の数を求める問題
- まず、(a,b)をbでソートする。すると
  - aとbが連結かどうかは直前に外した橋cにのみ影響されるようになる。
    - c~bはつながっているため。
  - なので`a<=c`のとき、連結してないので、橋を外さなくていい
  - 逆に`c<a`のとき、連結しているので、辺を外す。このとき外すのはb-1番目の橋
    - 後のことを考えると、連結している島の数は少ない方が有利なため。
- ソートがボトルネックなので `O(nlogn)`